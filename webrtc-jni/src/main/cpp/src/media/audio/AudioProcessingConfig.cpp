...
namespace jni
{
	namespace AudioProcessingConfig
	{
		...
		webrtc::AudioProcessing::Config::GainController2 toGainController2(JNIEnv * env, const JavaLocalRef<jobject> & javaType)
		{
			...
			const auto javaInputVolumeControllerClass = JavaClasses::get<JavaInputVolumeControllerClass>(env);
			...
			JavaObject inputVolumeController(env, gainControl.getObject(javaGainControlClass->inputVolumeController));
			...
			gainController.input_volume_controller.enabled = inputVolumeController.getBoolean(javaInputVolumeControllerClass->enabled);
		...
		JavaGainControllerDigitalClass::JavaGainControllerDigitalClass(JNIEnv* env)
		{
			...
			enabled = GetFieldID(env, cls, "enabled", "Z");
			inputVolumeController = GetFieldID(env, cls, "inputVolumeController", "L" PKG_AUDIO "AudioProcessingConfig$GainControllerDigital$InputVolumeController;");
		...
		{
			cls = FindClass(env, PKG_AUDIO"AudioProcessingConfig$GainControllerDigital$InputVolumeController");
