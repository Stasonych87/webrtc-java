...
#include "media/video/macos/objc/device_info_objc.h"
#include "media/video/macos/objc/rtc_video_capture_objc.h"
...
rtc::scoped_refptr<VideoCaptureModule> videocapturemodule::VideoCaptureImpl::Create(
...
VideoCaptureIos::VideoCaptureIos()
    : is_capturing_(false) {
  capability_.width = videocapturemodule::kDefaultWidth;
  capability_.height = videocapturemodule::kDefaultHeight;
  capability_.maxFPS = videocapturemodule::kDefaultFrameRate;
  capture_device_ = nil;
}
...
rtc::scoped_refptr<VideoCaptureModule> VideoCaptureIos::Create(
  ...
  RTC_LOG(LS_INFO) << "TEST Native: VideoCaptureIos start 1 ... ... ...";
//   rtc::scoped_refptr<VideoCaptureIos> capture_module(
//       new rtc::RefCountedObject<VideoCaptureIos>());
//   rtc::scoped_refptr<VideoCaptureIos> capture_module =
//       rtc::scoped_refptr<VideoCaptureIos>(new rtc::RefCountedObject<VideoCaptureIos>());
//   rtc::scoped_refptr<VideoCaptureIos> capture_module =
//       rtc::make_ref_counted<VideoCaptureIos>();
  auto capture_module = rtc::make_ref_counted<VideoCaptureIos>();

  const int32_t name_length = strlen(deviceUniqueIdUTF8);
  if (name_length > kVideoCaptureUniqueNameLength)
    return nullptr;

  RTC_LOG(LS_INFO) << "TEST Native: VideoCaptureIos start 2 ... ... ...";
...
RTC_LOG(LS_INFO) << "TEST Native: VideoCaptureIos start 3 ... ... ...";
  if (![capture_module->capture_device_
          setCaptureDeviceByUniqueId:
              [[NSString alloc] initWithCString:deviceUniqueIdUTF8
                                       encoding:NSUTF8StringEncoding]]) {
   RTC_LOG(LS_INFO) << "TEST Native: VideoCaptureIos return nullptr 4 ... ... ...";
    return nullptr;
  }
